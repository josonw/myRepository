...
def grep_form_into_table_name( file_path ):
    ...
    # 主要找出表之间依赖关系
    """
    pattern = re.compile(r'\s.*(from|join|table)\s([a-z._]*)\s.*',re.I)
    into_tab, form_tab = [], []
    for line in open(file_path):
        ...
        if match:
            ...
            if table.count('tmp') > 0 or table.count("temp"):
                continue
    ...
    form_tab = [ t for t in form_tab if t not in into_tab ]
    into_tab = [ t for t in into_tab if t.count('analysis')>0 or t.count('stage')>0  ]
...
insert_sql = """ INSERT INTO `job_config` (`pjid`, `pri`, `jobname`, `jobdesc`, `jobtype`, `tab_from`,
        ...
        VALUES """
job_path = 'D:\\data_oa\\byl_scs\jobs\stat\\pay_info'
