...
class agg_user_location_pg_day(BasePGStat):
    ...
    def stat(self):
        ...
        sql = """
        delete from user_location_country_info where fdate = date '%(ld_begin)s'
        """ % self.sql_dict
        self.append(sql)
        sql = """
        delete from user_location_province_info where fdate = date '%(ld_begin)s'
        """ % self.sql_dict
        self.append(sql)
        ...
        '''
        sql = """
            INSERT INTO user_location_country_info
            ...
            FROM user_location_info a
            LEFT JOIN COUNTRY_DIM b
                ON (CASE
                            WHEN a.fip_countrycode IN ('TW','HK','MO')
                                THEN 'CN'
                            ELSE a.fip_countrycode
        ...
        sql = """
        INSERT INTO user_location_province_info
        SELECT date '%(ld_begin)s' as  fdate ,
                    a.fgamefsk ,
                    a.fplatformfsk ,
                    a.fversionfsk ,
