...
class agg_game_activity_payret_data(BaseStat):
    """活动数据,时段数据
    """
    def create_tab(self):
        hql = """create external table if not exists analysis.game_activity_pay_retain_fct
                (
                fdate date,
                fgamefsk bigint,
                fplatformfsk bigint,
    ...
    def stat(self):
        ...
        # 创建临时表
        hql = """
        insert overwrite table analysis.game_activity_pay_retain_fct
        partition (dt )
        select a.fdate,
                fgamefsk,
                fplatformfsk,
        ...
        hql = """
        insert overwrite table analysis.game_activity_pay_retain_fct partition(dt='3000-01-01')
        select
        fdate,
        fgamefsk,
        fplatformfsk,
        ...
        f14dru,
        f30dru
        from analysis.game_activity_pay_retain_fct
        where dt >= '%(ld_30dayago)s'
        and dt < '%(ld_dayend)s'
        """ % query
        res = self.hq.exe_sql(hql)
...
#生成统计实例
a = agg_game_activity_payret_data(statDate)
a()
