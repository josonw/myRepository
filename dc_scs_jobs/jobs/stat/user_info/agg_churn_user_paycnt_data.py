...
class agg_churn_user_paycnt_data(BaseStat):
    """流失用户，历史付费次数
    """
    def create_tab(self):
        hql = """create external table if not exists analysis.user_lc_loss_paycnt_fct
                (
                fdate date,
                fgamefsk bigint,
                fplatformfsk bigint,
    ...
    def stat(self):
        ...
        # 创建临时表
        hql = """
        insert overwrite table analysis.user_lc_loss_paycnt_fct
        partition ( dt = '%(ld_daybegin)s' )
          select /*+ mapjoin(d) */
           ...
           count(if(churn_type = 'day' and days = 30, a.fuid, null)) f30eduserloss
            ...
            join (select b.fbpid, b.fuid, count(1) paycnt
            ...
            join analysis.bpid_platform_game_ver_map d
              on a.fbpid = d.fbpid
           group by d.fplatformfsk, d.fgamefsk, d.fversionfsk, d.fterminalfsk, paycnt
        """ % query
        res = self.hq.exe_sql(hql)
        if res != 0: return res
...
#生成统计实例
a = agg_churn_user_paycnt_data(statDate)
a()
