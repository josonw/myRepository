...
class agg_reg_user_source_data(BaseStat):
    """注册用户来源，广告用户，feed，push
    """
    def create_tab(self):
        hql = """create table if not exists analysis.reg_user_feed_ad
                (
                fdate date,
                fgamefsk bigint,
                fplatformfsk bigint,
    ...
    def stat(self):
        ...
        if res != 0: return res
        hql = """
        insert overwrite table analysis.reg_user_feed_ad
        partition (dt = '%(ld_daybegin)s')
          select '%(ld_daybegin)s' fdate,
                ...
                from (
                  ...
                  join analysis.bpid_platform_game_ver_map d  on a.fbpid = d.fbpid
                 where a.dt = '%(ld_daybegin)s'
                  group by d.fgamefsk,d.fplatformfsk,d.fversionfsk,d.fterminalfsk
                   ) tmp
...
if len(sys.argv) == 1:
    ...
    statDate = args[0]
#生成统计实例
a = agg_reg_user_source_data(statDate)
a()
