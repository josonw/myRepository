...
class agg_user_push_back_day(BaseStat):

    def create_tab(self):
        hql = """
        use analysis;
        create table if not exists analysis.user_push_back_fct
        (
          fdate            date,
          fplatformfsk        bigint,
          fgamefsk        bigint,
        ...
        location '/dw/analysis/user_push_back_fct'
        """
    ...
    def stat(self):
        ...
        hql = """    --
        insert overwrite table analysis.user_push_back_fct
        partition( dt="%(statdate)s" )
            select "%(statdate)s" fdate, fplatformfsk, fgamefsk, fversionfsk, fterminalfsk,
                nvl(ad.fsk, 1) fagefsk,
                case when cast(ud.fprofession as bigint) is null then 1
                    when cast(ud.fprofession as bigint)<1 then 1
                    when cast(ud.fprofession as bigint)>7 then 1
                    else cast(ud.fprofession as bigint)+4446684
                end foccupationalfsk,
            ...
            from (
                ...
                join stage.push_send_stg b
                    on a.fbpid = b.fbpid and a.fuid = b.fuid and cast(b.dt as string) in
...
#生成统计实例
a = agg_user_push_back_day(statDate, eid)
a()
