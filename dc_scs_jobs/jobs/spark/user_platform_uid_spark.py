...
-- fbpid:fplatform_uid 作为key
-- fbpid_fuid == fbpid:fuid 方便后续做join
CREATE EXTERNAL TABLE analysis.user_platform_uid_hbase
 (key string,
 fuid bigint,
 fbpid_fuid string
)
...
("hbase.columns.mapping" = ":key, b:fuid, b:fbpid_fuid")
TBLPROPERTIES ("hbase.table.name" = "user_platform_uid");
"""
...
def get_sql_list(date):
    sql_list = [
        # 注册信息
        """ select fbpid, fplatform_uid, fuid, concat(fbpid, ':', fuid) fbpid_fuid
            from stage.user_signup_stg
            where dt = '%(date)s' """,

        # 登陆信息
        """ select fbpid, fplatform_uid, max(fuid) fuid, max(concat(fbpid, ':', fuid)) fbpid_fuid
    ...
    sql_info_list = [("user_platform_uid", ["fbpid", "fplatform_uid"], sql % {'date':date} ) for sql in  sql_list]
...
def main():
    ...
    spark_app_name = "user_platform_uid"
    hbase_col_family = 'b:'
