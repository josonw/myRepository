...
class agg_match_join_user_joinret(BaseStat):
    """比赛场   报名留存
    """
    def create_tab(self):
        hql = """create table if not exists dcnew.match_join_user_joinret
                (
                fdate              date,
                fgamefsk           bigint,
                fplatformfsk       bigint,
    ...
    def stat(self):
        ...
        hql_list=[]
        hql = """
        drop table if exists work.match_join_user_joinret_%(num_begin)s;
        create table work.match_join_user_joinret_%(num_begin)s as
        select
            a.dt fdate,
        ...
        left join dim.user_match_join b
       ...
       group by
        ...
        hql = """
        insert overwrite table dcnew.match_join_user_joinret
        partition( dt )
        ...
        insert overwrite table dcnew.match_join_user_joinret partition(dt='3000-01-01')
            select  fdate, fgamefsk, fplatformfsk, fhallfsk, fsubgamefsk, fterminaltypefsk, fversionfsk, fchannelcode,
                    fpname, fsubname,
                    total_user total_user,
                    f1daycnt,f2daycnt,f3daycnt,f4daycnt,f5daycnt,f6daycnt,f7daycnt,f14daycnt,f30daycnt
        ...
        hql = """
        drop table if exists work.match_join_user_joinret_%(num_begin)s"""% query[0]
        hql_list.append(hql)
...
if len(sys.argv) == 1:
    ...
    statDate = args[0]
#生成统计实例
a = agg_match_join_user_joinret(statDate)
a()
