...
class agg_match_fplay_user_playret(BaseStat):
    """比赛场 新增参赛留存
    """
    def create_tab(self):
        hql = """create table if not exists dcnew.match_fplay_user_playret
                (
                fdate              date,
                fgamefsk           bigint,
                fplatformfsk       bigint,
    ...
    def stat(self):
        ...
        res = self.hq.exe_sql("""set hive.map.aggr=true; set hive.groupby.skewindata=true;""")
        if res != 0: return res
        ...
        hql_list=[]
        hql = """
        drop table if exists work.agg_match_fplay_user_playret_%(num_begin)s;
        create table work.agg_match_fplay_user_playret_%(num_begin)s as
        select
            a.dt fdate,
       ...
       group by
        ...
        hql = """
        insert overwrite table dcnew.match_fplay_user_playret
        partition( dt )
        ...
        from (
            ...
            from work.agg_match_fplay_user_playret_%(num_begin)s
            union all
            select  fdate, fgamefsk, fplatformfsk, fhallfsk, fsubgamefsk, fterminaltypefsk, fversionfsk, fchannelcode,
                    fpname, fsubname,
                    total_user total_user,
                    f1daycnt,f2daycnt,f3daycnt,f4daycnt,f5daycnt,f6daycnt,f7daycnt,f14daycnt,f30daycnt
            from dcnew.match_fplay_user_playret
        ...
        insert overwrite table dcnew.match_fplay_user_playret partition(dt='3000-01-01')
            select  fdate, fgamefsk, fplatformfsk, fhallfsk, fsubgamefsk, fterminaltypefsk, fversionfsk, fchannelcode,
                    fpname, fsubname,
                    total_user total_user,
                    f1daycnt,f2daycnt,f3daycnt,f4daycnt,f5daycnt,f6daycnt,f7daycnt,f14daycnt,f30daycnt
        ...
        hql = """
        drop table if exists work.agg_match_fplay_user_playret_%(num_begin)s"""% query[0]
        hql_list.append(hql)
...
if len(sys.argv) == 1:
    ...
    statDate = args[0]
#生成统计实例
a = agg_match_fplay_user_playret(statDate)
a()
