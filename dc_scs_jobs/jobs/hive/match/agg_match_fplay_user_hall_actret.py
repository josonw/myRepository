...
class agg_match_fplay_user_hall_actret(BaseStat):
    """比赛场 新增参赛大厅留存
    """
    def create_tab(self):
        hql = """create table if not exists dcnew.match_fplay_user_hall_actret
                (
                fdate              date,
                fgamefsk           bigint,
                fplatformfsk       bigint,
    ...
    def stat(self):
        ...
        hql_list=[]
        hql = """
        drop table if exists work.agg_match_fplay_user_hall_actret_%(num_begin)s;
        create table work.agg_match_fplay_user_hall_actret_%(num_begin)s as
        select
            a.dt fdate,
        ...
        join (
            select dt, fuid, fgamefsk, fplatformfsk, fhallfsk
            from dim.reg_user_array
            where (dt >= '%(ld_7dayago)s' and dt < '%(ld_dayend)s'
                    or dt='%(ld_14dayago)s'
                    or dt='%(ld_30dayago)s'
                    or dt='%(ld_60dayago)s')
              and fgame_id = %(null_int_group_rule)d
              and fterminaltypefsk = %(null_int_group_rule)d
              and fversionfsk = %(null_int_group_rule)d
              and fchannel_code = %(null_int_group_rule)d
            group by dt, fuid, fgamefsk, fplatformfsk, fhallfsk
              ) c
          on a.dt = c.dt
         and a.fuid = c.fuid
         and a.fgamefsk = c.fgamefsk
        ...
        left join dim.user_act_array b
         ...
         and b.fsubgamefsk = %(null_int_group_rule)d
         and b.fterminaltypefsk = %(null_int_group_rule)d
         and b.fversionfsk = %(null_int_group_rule)d
         and b.fchannelcode = %(null_int_group_rule)d
       ...
       group by
        ...
        hql = """
        insert overwrite table dcnew.match_fplay_user_hall_actret
        partition( dt )
        ...
        from (
            ...
            from work.agg_match_fplay_user_hall_actret_%(num_begin)s
            union all
            select  fdate, fgamefsk, fplatformfsk, fhallfsk, fsubgamefsk, fterminaltypefsk, fversionfsk, fchannelcode,
                    fpname, fsubname,
                    total_user total_user,
                    f1daycnt,f2daycnt,f3daycnt,f4daycnt,f5daycnt,f6daycnt,f7daycnt,f14daycnt,f30daycnt
            from dcnew.match_fplay_user_hall_actret
        ...
        insert overwrite table dcnew.match_fplay_user_hall_actret partition(dt='3000-01-01')
            select  fdate, fgamefsk, fplatformfsk, fhallfsk, fsubgamefsk, fterminaltypefsk, fversionfsk, fchannelcode,
                    fpname, fsubname,
                    total_user total_user,
                    f1daycnt,f2daycnt,f3daycnt,f4daycnt,f5daycnt,f6daycnt,f7daycnt,f14daycnt,f30daycnt
        ...
        hql = """
        drop table if exists work.agg_match_fplay_user_hall_actret_%(num_begin)s;"""% query[0]
        hql_list.append(hql)
...
if len(sys.argv) == 1:
    ...
    statDate = args[0]
#生成统计实例
a = agg_match_fplay_user_hall_actret(statDate)
a()
