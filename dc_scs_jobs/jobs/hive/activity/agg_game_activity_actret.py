...
class agg_game_activity_actret(BaseStatModel):
    """ 游戏活动活跃留存结果表"""
    def create_tab(self):
        hql = """
        create table if not exists dcnew.game_activity_actret
        (
            fdate                       date,
            fgamefsk                    bigint,
            fplatformfsk                bigint,
	...
	set hive.auto.convert.join=false;
        drop table if exists work.game_activity_actret_%(statdatenum)s;
        create table work.game_activity_actret_%(statdatenum)s as
        ...
        hql = """
         select fdate,
           ...
           from work.game_activity_actret_%(statdatenum)s a
        ...
        hql ="""
        drop table if exists work.game_activity_actret2_%(statdatenum)s;
        create table work.game_activity_actret2_%(statdatenum)s as
        %(sql_template)s
        """
        res = self.sql_exe(hql)
        if res != 0:return res
        ...
        hql = """
        insert overwrite table dcnew.game_activity_actret
        partition (dt)
        ...
        insert overwrite table dcnew.game_activity_actret partition(dt='3000-01-01')
                   select fdate, fgamefsk, fplatformfsk, fhallfsk, fsubgamefsk, fterminaltypefsk, fversionfsk, fchannelcode,
                    fact_id, fusernum,
                    f1daycnt,f2daycnt,f3daycnt,f4daycnt,f5daycnt,f6daycnt,f7daycnt,f14daycnt,f30daycnt
            from dcnew.game_activity_actret
...
#生成统计实例
a = agg_game_activity_actret(sys.argv[1:])
a()
