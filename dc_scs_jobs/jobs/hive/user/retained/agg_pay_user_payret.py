...
class agg_pay_user_payret(BaseStatModel):
    """付费用户，在其后一段时间内的付费留存
    """
    def create_tab(self):
        hql = """create table if not exists dcnew.pay_user_payret
                (
                fdate date,
                fgamefsk bigint,
                fplatformfsk bigint,
    ...
    def stat(self):
        ...
        hql = """
        drop table if exists work.pay_user_payret_1_%(statdatenum)s;
        create table work.pay_user_payret_1_%(statdatenum)s
        as
            select
                bm.fgamefsk, bm.fplatformfsk, bm.fhallfsk, bm.fterminaltypefsk, bm.fversionfsk, bm.hallmode,
                datediff('%(statdate)s', pay.dt) retday,
                pay.dt, pay.fgame_id, pay.fchannel_code, pay.fplatform_uid fuid
            from dim.user_pay_day  pay
            join dim.user_pay_day b
              on pay.fplatform_uid = b.fplatform_uid and pay.fbpid = b.fbpid and b.dt = '%(statdate)s'
        ...
        from work.pay_user_payret_1_%(statdatenum)s
        ...
        hql ="""
        drop table if exists work.pay_user_payret_2_%(statdatenum)s;
        create table work.pay_user_payret_2_%(statdatenum)s as
        %(sql_template)s
        """
        res = self.sql_exe(hql)
        if res != 0:return res
        ...
        hql = """
        insert overwrite table  dcnew.pay_user_payret
        partition( dt )
        ...
        select fdate, fgamefsk, fplatformfsk, fhallfsk, fsubgamefsk, fterminaltypefsk, fversionfsk, fchannelcode,
            ...
            from work.pay_user_payret_2_%(statdatenum)s
            union all
            SELECT fdate,fgamefsk, fplatformfsk, fhallfsk, fsubgamefsk, fterminaltypefsk, fversionfsk, fchannelcode,
                   f1daycnt,
                   f2daycnt,
        ...
        hql = """
        insert overwrite table dcnew.pay_user_payret partition(dt='3000-01-01')
        select
        fdate,
        ...
        res = self.sql_exe("""drop table if exists work.pay_user_payret_1_%(statdatenum)s;
                              drop table if exists work.pay_user_payret_2_%(statdatenum)s;""")
        if res != 0: return res


#生成统计实例
a = agg_pay_user_payret(sys.argv[1:])
a()
