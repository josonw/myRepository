...
class agg_province_match_monthly_info(BaseStatModel):
    def create_tab(self):
        hql = """--分省数据_月_赛事相关
        create table if not exists dcnew.province_match_monthly_info (
                fdate               date,
                fgamefsk            bigint,
                fplatformfsk        bigint,
                fhallfsk            bigint,
                ...
                fmatch_back_unum    bigint          comment '比赛用户次月留存',
                fhmatch_back_unum   bigint          comment '比赛用户大厅次月留存',
          ...
          location '/dw/dcnew/province_match_monthly_info'
        """
    ...
    def stat(self):
        ...
        hql = """
        drop table if exists work.province_match_m_tmp_1_%(statdatenum)s;
        create table work.province_match_m_tmp_1_%(statdatenum)s as
            ...
            from (--报名参加牌局比赛
                  select jg.fbpid,
                    ...
                    left join (select distinct fbpid, fuid
                                ...
                                where dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
                              ) ru
                      on jg.fbpid = ru.fbpid
                     and jg.fuid = ru.fuid
                   where jg.dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
                ...
                --参赛
                  select ug.fbpid,
                    ...
                    left join (select distinct fbpid, fuid
                     ...
                     and ug.fuid = ru.fuid
                   where ug.dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
                     and coalesce(fmatch_id,'0')<>'0'
                   union all
                --退赛
                  select fbpid,
                   ...
                   where qg.dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
                     and coalesce(fmatch_id,'0')<>'0'
                   union all
                --比赛场发放消耗
                  select fbpid,
                   ...
                   where mg.dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
                     and coalesce(fmatch_id,'0')<>'0'
                 ) a
            join dim.bpid_map c
              on a.fbpid=c.fbpid
        ...
        base_hql = """
                select fgamefsk,
                  ...
                  from work.province_match_m_tmp_1_%(statdatenum)s gs
              group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,
                       fprovince """

        #组合
        hql = ("""
                  drop table if exists work.province_match_m_tmp_%(statdatenum)s;
                create table work.province_match_m_tmp_%(statdatenum)s as """ +
        ...
        hql = """
        drop table if exists work.province_match_m_tmp_2_%(statdatenum)s;
        create table work.province_match_m_tmp_2_%(statdatenum)s as
          select /*+ MAPJOIN(c) */ c.fgamefsk,
                 c.fplatformfsk,
                 c.fhallfsk,
                 c.fterminaltypefsk,
                 c.fprovince,
                 coalesce(yug.fgame_id,%(null_int_report)d) fgame_id,
                 yug.fuid,
            ...
            from (select distinct fbpid, fuid, fgame_id
                   ...
                   where dt between '%(ld_1month_ago_begin)s' and '%(ld_1month_ago_end)s'
                     and coalesce(fmatch_id,'0')<>'0'
                 ) yug
        ...
        base_hql = """
                select fgamefsk,
                  ...
                  from work.province_match_m_tmp_2_%(statdatenum)s gs
              group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,
                       fprovince """

        #组合
        hql = ("""
                  insert into table work.province_match_m_tmp_%(statdatenum)s """ +
                base_hql + """%(extend_group_1)s """ +
                base_hql + """%(extend_group_2)s """ +
                base_hql + """%(extend_group_3)s """ +
                base_hql + """%(extend_group_4)s """ )% query
        ...
        hql = """
        drop table if exists work.province_match_m_tmp_3_%(statdatenum)s;
        create table work.province_match_m_tmp_3_%(statdatenum)s as
          select /*+ MAPJOIN(c) */ c.fgamefsk,
                 c.fplatformfsk,
                 c.fhallfsk,
                 c.fterminaltypefsk,
                 c.fprovince,
                 coalesce(ps.fgame_id,%(null_int_report)d) fgame_id,
                 ug.fuid,
                 ps.fplatform_uid,
                 ps.fpaycnt,
                 ps.dip
            from (select a.dt,
                    ...
                    left join stage.user_generate_order_stg t2
                     ...
                     and a.dt = t2.dt
                     and t2.dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
                    ...
                    join dim.bpid_map c
                      on a.fbpid=c.fbpid
                     and c.fgamefsk = 4132314431  --地方棋牌
                   where a.dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
                   group by a.dt, a.fbpid, a.fplatform_uid,t2.fgame_id,e.fuid
                 ) ps
            join (select distinct dt, fbpid, fuid
                    from stage.user_gameparty_stg
                   where dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
                     and coalesce(fmatch_id,'0')<>'0'
                 ) ug
        ...
        base_hql = """
                select fgamefsk,
                  ...
                  from work.province_match_m_tmp_3_%(statdatenum)s gs
              group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,
                       fprovince """
        ...
        hql = """insert overwrite table dcnew.province_match_monthly_info
                  ...
                  select '%(ld_month_begin)s' fdate,
                    ...
                    from work.province_match_m_tmp_%(statdatenum)s gs
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.province_match_m_tmp_1_%(statdatenum)s;
                 drop table if exists work.province_match_m_tmp_2_%(statdatenum)s;
                 drop table if exists work.province_match_m_tmp_3_%(statdatenum)s;
                 drop table if exists work.province_match_m_tmp_%(statdatenum)s;
        """
...
#生成统计实例
a = agg_province_match_monthly_info(sys.argv[1:])
a()
