...
class agg_province_user_monthly_info(BaseStatModel):
    def create_tab(self):
        hql = """--分省数据_用户相关
        create table if not exists dcnew.province_user_monthly_info (
                fdate               date,
                fgamefsk            bigint,
                fplatformfsk        bigint,
                fhallfsk            bigint,
                ...
                fgame_back_unum     bigint          comment '游戏内次月留存',
                fgame_act_unum      bigint          comment '游戏内活跃人数',
                fhall_back_unum     bigint          comment '大厅内次月留存',
          ...
          location '/dw/dcnew/province_user_monthly_info'
        """
    ...
    def stat(self):
        ...
        hql = """
        drop table if exists work.province_user_m_tmp_1_%(statdatenum)s;
        create table work.province_user_m_tmp_1_%(statdatenum)s as
          select /*+ MAPJOIN(d) */ c.fgamefsk,
            ...
            left join (select distinct fbpid, fuid, fpartner_info
        ...
        base_hql = """
                select fgamefsk,
                       ...
                       0 favg_online,
                       0 favg_play
                  from work.province_user_m_tmp_1_%(statdatenum)s gs
              group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,
                       fprovince """

        hql = ("""
                  drop table if exists work.province_user_m_tmp_%(statdatenum)s;
                create table work.province_user_m_tmp_%(statdatenum)s as """ +
        ...
        hql = """
        drop table if exists work.province_user_m_tmp_2_%(statdatenum)s;
        create table work.province_user_m_tmp_2_%(statdatenum)s as
          select c.fgamefsk,
                 c.fplatformfsk,
                 c.fhallfsk,
                 c.fterminaltypefsk,
            ...
            left join (select distinct fbpid, fuid, fgame_id
              ...
              on t1.fbpid = t2.fbpid
             and t1.fuid = t2.fuid
            left join (select distinct fbpid, fuid, ffeed_as
                         from stage.feed_clicked_stg
                        where dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
                      ) t3
              on t1.fbpid = t3.fbpid
             and t1.fuid = t3.fuid
            join dim.bpid_map c
              on t1.fbpid=c.fbpid
             and c.fgamefsk = 4132314431  --地方棋牌
           where t1.dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
        ...
        base_hql = """
                select fgamefsk,
                       ...
                       0 favg_online,
                       0 favg_play
                  from work.province_user_m_tmp_2_%(statdatenum)s gs
              group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,
                       fprovince """

        hql = (""" insert into table work.province_user_m_tmp_%(statdatenum)s """ +
               base_hql + """%(extend_group_1)s """ +
               base_hql + """%(extend_group_2)s """ +
               base_hql + """%(extend_group_3)s """ +
               base_hql + """%(extend_group_4)s """ )% query
        ...
        hql = """insert overwrite table dcnew.province_user_monthly_info
                  ...
                  select '%(ld_month_begin)s' fdate,
                    ...
                    from work.province_user_m_tmp_%(statdatenum)s gs
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.province_user_m_tmp_1_%(statdatenum)s;
                 drop table if exists work.province_user_m_tmp_2_%(statdatenum)s;
                 drop table if exists work.province_user_m_tmp_%(statdatenum)s;
        """
        res = self.sql_exe(hql)
...
#生成统计实例
a = agg_province_user_monthly_info(sys.argv[1:])
a()
