...
class agg_province_pay_monthly_info(BaseStatModel):
    def create_tab(self):
        hql = """--分省数据_月_付费相关
        create table if not exists dcnew.province_pay_monthly_info (
                fdate               date,
                fgamefsk            bigint,
                fplatformfsk        bigint,
                fhallfsk            bigint,
          ...
          location '/dw/dcnew/province_pay_monthly_info'
        """
    ...
    def stat(self):
        ...
        hql = """
        drop table if exists work.province_pay_m_tmp_1_%(statdatenum)s;
        create table work.province_pay_m_tmp_1_%(statdatenum)s as
          select /*+ MAPJOIN(d) */ distinct c.fgamefsk,
            ...
            left join stage.payment_stream_stg t3  --成功订单
             ...
             and t3.dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
            left join dim.user_pay t4  --首付
              on t3.fbpid = t4.fbpid
             and t3.fplatform_uid = t4.fplatform_uid
             and t4.dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
            join dim.bpid_map c
              on t1.fbpid=c.fbpid
        ...
        base_hql = """
                select fgamefsk,
                  ...
                  from work.province_pay_m_tmp_1_%(statdatenum)s gs
              group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,
                       fprovince """

        #组合
        hql = ("""
                  drop table if exists work.province_pay_m_tmp_%(statdatenum)s;
                create table work.province_pay_m_tmp_%(statdatenum)s as """ +
        ...
        hql = """insert overwrite table dcnew.province_pay_monthly_info
                  ...
                  select '%(ld_month_begin)s' fdate,
                    ...
                    from work.province_pay_m_tmp_%(statdatenum)s gs
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.province_pay_m_tmp_1_%(statdatenum)s;
                 drop table if exists work.province_pay_m_tmp_%(statdatenum)s;
        """
        res = self.sql_exe(hql)
        if res != 0:
...
#生成统计实例
a = agg_province_pay_monthly_info(sys.argv[1:])
a()
