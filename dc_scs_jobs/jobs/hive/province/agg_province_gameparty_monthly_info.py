...
class agg_province_gameparty_monthly_info(BaseStatModel):
    def create_tab(self):
        hql = """--分省数据_月_牌局场次相关
        create table if not exists dcnew.province_gameparty_monthly_info (
                fdate               date,
                fgamefsk            bigint,
                fplatformfsk        bigint,
                fhallfsk            bigint,
                ...
                )comment '分省数据_月_牌局场次相关'
                partitioned by(dt date)
          location '/dw/dcnew/province_gameparty_monthly_info'
        """
    ...
    def stat(self):
        ...
        hql = """--玩牌人数、玩牌人次、新增用户数
        drop table if exists work.province_gameparty_m_tmp_1_%(statdatenum)s;
        create table work.province_gameparty_m_tmp_1_%(statdatenum)s as
          select /*+ MAPJOIN(c) */ c.fgamefsk,
            ...
            from (select dt, fbpid, fuid, fcoins_type, fgame_id, count(1) play_cnt
                    from stage.user_gameparty_stg
                   where dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
                   group by dt, fbpid, fuid, fcoins_type, fgame_id
                 ) a
            left join (select distinct dt, fbpid, fuid
        ...
        base_hql = """
                select fgamefsk,
                       ...
                       0 frupt_pay_unum,
                       0 frupt_pay_cnt,
                       cast (0 as decimal(20,2)) frupt_pay_income
                  from work.province_gameparty_m_tmp_1_%(statdatenum)s gs
              group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fcoins_type,
                       fprovince """

        #组合
        hql = ("""
                  drop table if exists work.province_gameparty_m_tmp_%(statdatenum)s;
                create table work.province_gameparty_m_tmp_%(statdatenum)s as """ +
        ...
        drop table if exists work.province_gameparty_m_tmp_2_%(statdatenum)s;
        create table work.province_gameparty_m_tmp_2_%(statdatenum)s as
          select /*+ MAPJOIN(c) */ c.fgamefsk,
                 c.fplatformfsk,
                 c.fhallfsk,
                 c.fterminaltypefsk,
        ...
        base_hql = """
                select fgamefsk,
                       ...
                       0 frupt_pay_cnt,
                       0 frupt_pay_income
                  from work.province_gameparty_m_tmp_2_%(statdatenum)s gs
              group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fcoins_type,
                       fprovince """

        #组合
        hql = ("""
                 insert into table work.province_gameparty_m_tmp_%(statdatenum)s """ +
                base_hql + """%(extend_group_1)s """ +
                base_hql + """%(extend_group_2)s """ +
                base_hql + """%(extend_group_3)s """ +
                base_hql + """%(extend_group_4)s """ )% query
        ...
        hql = """--破产牌局数、总牌局数、总玩牌时长
        drop table if exists work.province_gameparty_m_tmp_3_%(statdatenum)s;
        create table work.province_gameparty_m_tmp_3_%(statdatenum)s as
          select /*+ MAPJOIN(c) */ distinct c.fgamefsk,
            ...
            left join stage.user_gameparty_stg t
             ...
             and a.dt  = t.dt
             and t.dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
             and t.fis_bankrupt = 1
            left join (select distinct dt, fbpid, fuid, fplatform_uid
                         from dim.user_pay_day
                        where dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
                      ) b
              on a.fbpid = b.fbpid
            ...
            left join stage.payment_stream_stg d
             ...
             and b.fbpid = d.fbpid
             and b.dt  = d.dt
             and d.dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
            join dim.bpid_map c
              on a.fbpid=c.fbpid
             and c.fgamefsk = 4132314431  --地方棋牌
           where a.dt between '%(ld_month_begin)s' and '%(ld_month_end)s'
        ...
        base_hql = """
                select fgamefsk,
                  ...
                  from work.province_gameparty_m_tmp_3_%(statdatenum)s gs
              group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fcoins_type,
                       fprovince """
        ...
        hql = """insert overwrite table dcnew.province_gameparty_monthly_info
                  ...
                  select '%(ld_month_begin)s' fdate,
                    ...
                    from work.province_gameparty_m_tmp_%(statdatenum)s gs
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.province_gameparty_m_tmp_1_%(statdatenum)s;
                 drop table if exists work.province_gameparty_m_tmp_2_%(statdatenum)s;
                 drop table if exists work.province_gameparty_m_tmp_3_%(statdatenum)s;
                 drop table if exists work.province_gameparty_m_tmp_%(statdatenum)s;
        """
...
#生成统计实例
a = agg_province_gameparty_monthly_info(sys.argv[1:])
a()
