...
class load_user_gameparty_nomatch(BaseStatModel):

#将原过程拆分为三个，分别运行，减少单个脚本资源占用，提高整体速度
#前置依赖L:load_gameparty_info
#本脚本生成 dim.user_gameparty_nomatch
    ...
    dim.user_gameparty_nomatch 以（uid，pname，subname）+ 7个粒度 + hallmode （ load_gameparty ），剔除比赛场牌局
     """
    def create_tab(self):

        hql = """
        create table if not exists dim.user_gameparty_nomatch
        (
         fbpid varchar(50),
          fgame_id bigint,
          ...
          fmax_win_amt        decimal(32),         --最大赢得的游戏币
          fmax_lose_amt       decimal(32),         --最大输掉的游戏币
    ...
    def stat(self):
        ...
        hql ="""
        insert overwrite table dim.user_gameparty_nomatch
        ...
        select
            ...
            max( case when ug.fgamecoins > 0 then fgamecoins else 0 end) fmax_win_amt,
            max( case when ug.fgamecoins < 0 then abs(fgamecoins) else 0 end) fmax_lose_amt,
...
#生成统计实例
a = load_user_gameparty_nomatch(sys.argv[1:])
a()
