...
class agg_gameparty_pname_payuser_detail(BaseStat):
    def create_tab(self):

        hql = """
        use analysis;
        create table if not exists dcnew.gameparty_pname_payuser_detail
        (
          fdate           date,
          fgamefsk           bigint,
          fplatformfsk       bigint,
    ...
    def stat(self):
        ...
        hql = """    -- 把临时表数据处理后弄到正式表上
        insert overwrite table dcnew.gameparty_pname_payuser_detail partition( dt="%(ld_begin)s" )
...
# 生成统计实例
a = agg_gameparty_pname_payuser_detail(statDate, eid)
a()
