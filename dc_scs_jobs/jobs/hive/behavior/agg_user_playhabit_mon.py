...
class agg_user_playhabit_mon(BaseStatModel):
    def create_tab(self):

#将周和月数据分开，先计算周数据，然后计算月数据，提高速度
#
#本脚本计算月数据，周数据为 agg_user_playhabit
    ...
    def stat(self):
        ...
        hql = """--取各类用户,活跃天数分布
        drop table if exists work.user_playhabit_tmp_m_%(statdatenum)s;
        create table work.user_playhabit_tmp_m_%(statdatenum)s as
       select /*+ MAPJOIN(d) */ distinct d.fgamefsk,
        ...
        hql = """--汇总不同组合下的活跃天数等数据
                select fgamefsk,
                       ...
                       uid_pay,
                       uid_match
                  from work.user_playhabit_tmp_m_%(statdatenum)s gs
        ...
        hql = """
        drop table if exists work.user_playhabit_tmp_m_s_%(statdatenum)s;
        create table work.user_playhabit_tmp_m_s_%(statdatenum)s as
        %(sql_template)s;
        """
        res = self.sql_exe(hql)
        if res != 0:
        ...
        hql = """--插入目标表
         insert into table dcnew.user_playhabit partition( dt="%(statdate)s" )
                select "%(statdate)s" fdate,
                  ...
                  from work.user_playhabit_tmp_m_s_%(statdatenum)s gs
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.user_playhabit_tmp_m_%(statdatenum)s;
                 drop table if exists work.user_playhabit_tmp_m_s_%(statdatenum)s;
        """
        res = self.sql_exe(hql)
        if res != 0:
...
#生成统计实例
a = agg_user_playhabit_mon(sys.argv[1:])
a()
