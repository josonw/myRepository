...
class agg_user_true_act(BaseStatModel):
    def create_tab(self):

        hql = """--用户阶段活跃数
        create table if not exists dcnew.user_true_act (
               fdate               date,
               fgamefsk            bigint,
               fplatformfsk        bigint,
               fhallfsk            bigint,
        ...
        location '/dw/dcnew/user_true_act'
        """
    ...
    def stat(self):
        ...
        hql = """--
        drop table if exists work.user_true_act_tmp_%(statdatenum)s;
        create table work.user_true_act_tmp_%(statdatenum)s as
               select d.fgamefsk,
                      d.fplatformfsk,
                      d.fhallfsk,
                      d.fterminaltypefsk,
        ...
        hql = """
                select "%(statdate)s" fdate,
                  ...
                  from work.user_true_act_tmp_%(statdatenum)s gs
        ...
        hql = """
        insert overwrite table dcnew.user_true_act
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.user_true_act_tmp_%(statdatenum)s;
        """
        res = self.sql_exe(hql)
        if res != 0:
            return res
...
#生成统计实例
a = agg_user_true_act(sys.argv[1:])
a()
