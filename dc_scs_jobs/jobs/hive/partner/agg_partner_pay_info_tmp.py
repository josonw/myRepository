...
class agg_partner_pay_info_tmp(BaseStatModel):
    def create_tab(self):

        hql = """--代理渠道付费数据
        create table if not exists dcnew.partner_pay_info_tmp (
               fdate               date,
               fgamefsk            bigint,
               fplatformfsk        bigint,
               fhallfsk            bigint,
        ...
        location '/dw/dcnew/partner_pay_info_tmp'
        """
    ...
    def stat(self):
        ...
        drop table if exists work.partner_pay_info_tmp_tmp_b_%(statdatenum)s;
        create table work.partner_pay_info_tmp_tmp_b_%(statdatenum)s as
        ...
        hql = """--下单数、付费用户数、付费次数、付费金额、首付用户数、首付次数、首付金额
                select '%(statdate)s' fdate,
                  ...
                  from work.partner_pay_info_tmp_tmp_b_%(statdatenum)s gs
        ...
        hql = """
        drop table if exists work.partner_pay_info_tmp_tmp_b_agg_%(statdatenum)s;
        create table work.partner_pay_info_tmp_tmp_b_agg_%(statdatenum)s as
        %(sql_template)s;
        """
        res = self.sql_exe(hql)
        if res != 0:
        ...
        hql = """--首付金额
        drop table if exists work.partner_pay_info_tmp_tmp_f_%(statdatenum)s;
        create table work.partner_pay_info_tmp_tmp_f_%(statdatenum)s as
        ...
        hql = """--首付金额
                select '%(statdate)s' fdate,
                  ...
                  from work.partner_pay_info_tmp_tmp_f_%(statdatenum)s gs
                 where hallmode = %(hallmode)s
              group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fversionfsk, fchannel_code,
                       fpartner_info
        """
        ...
        hql = """
        insert into table work.partner_pay_info_tmp_tmp_b_agg_%(statdatenum)s
        %(sql_template)s;
        """
        res = self.sql_exe(hql)
        if res != 0:
        ...
        hql = """--活跃
        drop table if exists work.partner_pay_info_tmp_tmp_a_%(statdatenum)s;
        create table work.partner_pay_info_tmp_tmp_a_%(statdatenum)s as
          select /*+ MAPJOIN(c) */ c.fgamefsk,
                 c.fplatformfsk,
                 c.fhallfsk,
                 c.fterminaltypefsk,
        ...
        hql = """--活跃用户数
                select '%(statdate)s' fdate,
                       ...
                       0 ffpay_cnt,
                       0 ffpay_income
                  from work.partner_pay_info_tmp_tmp_a_%(statdatenum)s gs
                 where hallmode = %(hallmode)s
              group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fversionfsk, fchannel_code,
                       fpartner_info
        """
        ...
        hql = """insert overwrite table dcnew.partner_pay_info_tmp
                  ...
                  select fdate,
                    ...
                    from work.partner_pay_info_tmp_tmp_b_agg_%(statdatenum)s gs
...
#生成统计实例
a = agg_partner_pay_info_tmp(sys.argv[1:])
a()
