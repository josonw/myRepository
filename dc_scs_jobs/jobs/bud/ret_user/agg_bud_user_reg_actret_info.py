...
class agg_bud_user_reg_actret_info(BaseStatModel):
    def create_tab(self):
        hql = """
        create table if not exists bud_dm.bud_user_reg_actret_info (
               fdate               date,
               fgamefsk            bigint,
               fplatformfsk        bigint,
               fhallfsk            bigint,
        ...
        location '/dw/bud_dm/bud_user_reg_actret_info';
        """
    ...
    def stat(self):
        ...
        # 取基础数据
        hql = """--子游戏新增
            drop table if exists work.bud_user_reg_actret_info_tmp_1_%(statdatenum)s;
          create table work.bud_user_reg_actret_info_tmp_1_%(statdatenum)s as
        ...
        base_hql = """
          select "%(statdate)s" fdate
            ...
            from work.bud_user_reg_actret_info_tmp_1_%(statdatenum)s t
        ...
        # 组合
        hql = (
            """insert overwrite table bud_dm.bud_user_reg_actret_info
        ...
        # 取基础数据
        hql = """--非子游戏新增
            drop table if exists work.bud_user_reg_actret_info_tmp_2_%(statdatenum)s;
          create table work.bud_user_reg_actret_info_tmp_2_%(statdatenum)s as
          select /*+ MAPJOIN(tt) */ tt.fgamefsk
                 ,tt.fplatformfsk
                 ,tt.fhallfsk
        ...
        base_hql = """
          select "%(statdate)s" fdate
                 ...
                 ,count(distinct act_uid) fhall_ret_unum
            from work.bud_user_reg_actret_info_tmp_2_%(statdatenum)s t
           group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fversionfsk
                    ,flast_date
         """
        ...
        # 组合
        hql = (
            """insert into table bud_dm.bud_user_reg_actret_info
                      partition(dt='%(statdate)s') """ +
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.bud_user_reg_actret_info_tmp_1_%(statdatenum)s;
                 drop table if exists work.bud_user_reg_actret_info_tmp_2_%(statdatenum)s;
        """
        res = self.sql_exe(hql)
        if res != 0:
...
# 生成统计实例
a = agg_bud_user_reg_actret_info(sys.argv[1:])
a()
