...
class agg_bud_act_actret_weekly_info(BaseStatModel):
    def create_tab(self):
        hql = """
        create table if not exists bud_dm.bud_act_actret_weekly_info (
               fdate               date,
               fgamefsk            bigint,
               fplatformfsk        bigint,
               fhallfsk            bigint,
        ...
        location '/dw/bud_dm/bud_act_actret_weekly_info';
        """
    ...
    def stat(self):
        ...
        # 取基础数据
        hql = ("""--子游戏新增
            drop table if exists work.bud_act_actret_weekly_info_tmp_2_%(statdatenum)s;
          create table work.bud_act_actret_weekly_info_tmp_2_%(statdatenum)s as
        ...
        base_hql = """
          select "%(ld_weekbegin)s" fdate
            ...
            from work.bud_act_actret_weekly_info_tmp_2_%(statdatenum)s t
        ...
        # 组合
        hql = (
            """insert overwrite table bud_dm.bud_act_actret_weekly_info
        ...
        # 取基础数据
        hql = ("""--非子游戏新增
            drop table if exists work.bud_act_actret_weekly_info_tmp_3_%(statdatenum)s;
          create table work.bud_act_actret_weekly_info_tmp_3_%(statdatenum)s as
               select /*+ MAPJOIN(tt) */ tt.fgamefsk
                      ,tt.fplatformfsk
                      ,tt.fhallfsk
        ...
        base_hql = """
          select "%(ld_weekbegin)s" fdate
                 ...
                 ,count(distinct act_uid) fhall_ret_unum
            from work.bud_act_actret_weekly_info_tmp_3_%(statdatenum)s t
           group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fversionfsk
                    ,flast_date
         """
        ...
        # 组合
        hql = (
            """insert into table bud_dm.bud_act_actret_weekly_info
                      partition(dt='%(ld_weekbegin)s') """ +
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.bud_act_actret_weekly_info_tmp_2_%(statdatenum)s;
                 drop table if exists work.bud_act_actret_weekly_info_tmp_3_%(statdatenum)s;
        """
        res = self.sql_exe(hql)
        if res != 0:
...
# 生成统计实例
a = agg_bud_act_actret_weekly_info(sys.argv[1:])
a()
