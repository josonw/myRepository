...
class agg_bud_user_pay_actret_info(BaseStatModel):
    def create_tab(self):
        hql = """
        create table if not exists bud_dm.bud_user_pay_actret_info (
               fdate               date,
               fgamefsk            bigint,
               fplatformfsk        bigint,
               fhallfsk            bigint,
               ...
               fversionfsk         bigint,
               flast_date          date        comment '付费日期',
               fpay_unum           bigint      comment '当日付费用户',
               fact_ret_unum       bigint      comment '活跃留存',
               fpay_ret_unum       bigint      comment '付费留存',
               fgpay_ret_unum      bigint      comment '子游戏付费留存'
               )comment '付费用户活跃留存'
               partitioned by(dt date)
        location '/dw/bud_dm/bud_user_pay_actret_info';
        """
    ...
    def stat(self):
        ...
        # 取基础数据
        hql = """--付费
            drop table if exists work.bud_user_pay_actret_info_tmp_1_%(statdatenum)s;
          create table work.bud_user_pay_actret_info_tmp_1_%(statdatenum)s as
        ...
        base_hql = """
          select "%(statdate)s" fdate
                 ...
                 ,flast_date
                 ,count(distinct pay_uid) fpay_unum
                 ,count(distinct act_uid) fact_ret_unum
                 ,count(distinct pay_ret_uid) fpay_ret_unum
                 ,count(distinct case when is_game = 1 then pay_ret_uid end) fgpay_ret_unum
            from work.bud_user_pay_actret_info_tmp_1_%(statdatenum)s t
        ...
        # 组合
        hql = (
            """insert overwrite table bud_dm.bud_user_pay_actret_info
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.bud_user_pay_actret_info_tmp_1_%(statdatenum)s;
        """
        res = self.sql_exe(hql)
        if res != 0:
            return res
...
# 生成统计实例
a = agg_bud_user_pay_actret_info(sys.argv[1:])
a()
