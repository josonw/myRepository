...
class agg_bud_user_act_actret_info(BaseStatModel):
    def create_tab(self):
        hql = """
        create table if not exists bud_dm.bud_user_act_actret_info (
               fdate               date,
               fgamefsk            bigint,
               fplatformfsk        bigint,
               fhallfsk            bigint,
        ...
        location '/dw/bud_dm/bud_user_act_actret_info';
        """
    ...
    def stat(self):
        ...
        # 取基础数据
        hql = """--子游戏与大厅活跃
            drop table if exists work.bud_user_act_actret_info_tmp_1_%(statdatenum)s;
          create table work.bud_user_act_actret_info_tmp_1_%(statdatenum)s as
        ...
        base_hql = """
          select "%(statdate)s" fdate
            ...
            from work.bud_user_act_actret_info_tmp_1_%(statdatenum)s t
        ...
        # 组合
        hql = (
            """insert overwrite table bud_dm.bud_user_act_actret_info
        ...
        # 组合
        hql = (
            """insert into table bud_dm.bud_user_act_actret_info
                      partition(dt='%(statdate)s') """ +
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.bud_user_act_actret_info_tmp_1_%(statdatenum)s;
        """
        res = self.sql_exe(hql)
        if res != 0:
            return res
...
# 生成统计实例
a = agg_bud_user_act_actret_info(sys.argv[1:])
a()
