...
class agg_bud_all_currencies_dis(BaseStatModel):
    def create_tab(self):
        hql = """
    ...
    def stat(self):
        ...
        # 汇总 --总金额
        hql = """
         insert overwrite table bud_dm.bud_all_currencies_dis
         partition( dt="%(statdate)s" )
          select "%(statdate)s" fdate
                 ...
                 ,sum(case when fuser_type = 'act_user' then coalesce(t.fbank_gamecoins_num, 0)+ coalesce(t.fcurrencies_num, 0) end) fact_num   --活跃货币数
                 ,count(case when fuser_type = 'reg_user' then fuid end) freg_unum   --新增用户数
                 ,sum(case when fuser_type = 'reg_user' then coalesce(t.fbank_gamecoins_num, 0)+ coalesce(t.fcurrencies_num, 0) end) freg_num   --新增货币数
                 ,count(case when fuser_type = 'pay_user' then fuid end) fpay_unum   --付费用户数
                 ,sum(case when fuser_type = 'pay_user' then coalesce(t.fbank_gamecoins_num, 0)+ coalesce(t.fcurrencies_num, 0) end) fpay_num   --付费货币数
                 ,count(case when fuser_type = 'rupt_user' then fuid end) fbankrupt_unum   --破产用户数
                 ,sum(case when fuser_type = 'rupt_user' then coalesce(t.fbank_gamecoins_num, 0)+ coalesce(t.fcurrencies_num, 0) end) fbankrupt_num   --破产货币数
                 ,count(case when fuser_type = 'all_user' then fuid end) fall_unum   --所有用户数
                 ,sum(case when fuser_type = 'all_user' then coalesce(t.fbank_gamecoins_num, 0)+ coalesce(t.fcurrencies_num, 0) end) fall_num   --所有货币数
                 ,count(case when fuser_type = 'paid_user' then fuid end) fpaid_unum   --历史付费活跃用户数
                 ,sum(case when fuser_type = 'paid_user' then coalesce(t.fbank_gamecoins_num, 0)+ coalesce(t.fcurrencies_num, 0) end) fpaid_num   --历史付费活跃货币数
                 ,all_flft flft   --区间下界
                 ,all_flft frgt   --区间上界
            from work.bud_all_currencies_dis_tmp_%(statdatenum)s t
        ...
        # 汇总 --总金额
        hql = """
         insert overwrite table bud_dm.bud_bring_currencies_dis
         partition( dt="%(statdate)s" )
          select "%(statdate)s" fdate
                 ,fgamefsk
                 ,fplatformfsk
                 ...
                 ,fcurrencies_type
                 ,count(case when fuser_type = 'act_user' then fuid end) fact_unum   --活跃用户数
                 ,sum(case when fuser_type = 'act_user' then coalesce(t.fcurrencies_num, 0) end) fact_num   --活跃货币数
                 ,count(case when fuser_type = 'reg_user' then fuid end) freg_unum   --新增用户数
                 ,sum(case when fuser_type = 'reg_user' then coalesce(t.fcurrencies_num, 0) end) freg_num   --新增货币数
                 ,count(case when fuser_type = 'pay_user' then fuid end) fpay_unum   --付费用户数
                 ,sum(case when fuser_type = 'pay_user' then coalesce(t.fcurrencies_num, 0) end) fpay_num   --付费货币数
                 ,count(case when fuser_type = 'rupt_user' then fuid end) fbankrupt_unum   --破产用户数
                 ,sum(case when fuser_type = 'rupt_user' then coalesce(t.fcurrencies_num, 0) end) fbankrupt_num   --破产货币数
                 ,count(case when fuser_type = 'all_user' then fuid end) fall_unum   --所有用户数
                 ,sum(case when fuser_type = 'all_user' then coalesce(t.fcurrencies_num, 0) end) fall_num   --所有货币数
                 ,count(case when fuser_type = 'paid_user' then fuid end) fpaid_unum   --历史付费活跃用户数
                 ,sum(case when fuser_type = 'paid_user' then coalesce(t.fcurrencies_num, 0) end) fpaid_num   --历史付费活跃货币数
                 ,br_flft flft   --区间下界
                 ,br_flft frgt   --区间上界
            from work.bud_all_currencies_dis_tmp_%(statdatenum)s t
           group by t.fgamefsk,t.fplatformfsk,t.fhallfsk,t.fgame_id,t.fterminaltypefsk,t.fversionfsk
        ...
        # 汇总 --总金额
        hql = """
         insert overwrite table bud_dm.bud_bank_currencies_dis
         partition( dt="%(statdate)s" )
          select "%(statdate)s" fdate
                 ,fgamefsk
                 ,fplatformfsk
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.bud_all_currencies_dis_tmp_2_%(statdatenum)s;
...
# 生成统计实例
a = agg_bud_all_currencies_dis(sys.argv[1:])
a()
