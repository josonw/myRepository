...
class agg_bud_partner_detail(BaseStatModel):
    def create_tab(self):
        hql = """
        create table if not exists bud_dm.bud_partner_detail (
               fdate               date,
               fgamefsk            bigint,
               fplatformfsk        bigint,
               fhallfsk            bigint,
               ...
               fpartner            varchar(100)  comment  '代理商id',
        ...
        location '/dw/bud_dm/bud_partner_detail';
        """
    ...
    def stat(self):
        ...
        # 两组组合，共4种
        extend_group_1 = """
                grouping sets ((fgamefsk, fplatformfsk, fhallfsk, fgame_id, fterminaltypefsk, fversionfsk, fsource_path, fpromoter, fpartner),
                               (fgamefsk, fplatformfsk, fhallfsk, fgame_id, fsource_path, fpromoter, fpartner),
                               (fgamefsk, fgame_id, fsource_path, fpromoter, fpartner) ) """

        extend_group_2 = """
                grouping sets ((fgamefsk, fplatformfsk, fhallfsk, fterminaltypefsk, fversionfsk, fsource_path, fpromoter, fpartner),
                               (fgamefsk, fplatformfsk, fhallfsk, fsource_path, fpromoter, fpartner),
                               (fgamefsk, fplatformfsk, fsource_path, fpromoter, fpartner) ) """
        ...
        # 取基础数据
        hql = """--取昨日新增活跃用户
            drop table if exists work.bud_partner_detail_tmp_2_%(statdatenum)s;
          create table work.bud_partner_detail_tmp_2_%(statdatenum)s as
          select t1.fbpid
        ...
        # 取基础数据
        hql = """--取当日新增活跃玩牌付费用户
            drop table if exists work.bud_partner_detail_tmp_3_%(statdatenum)s;
          create table work.bud_partner_detail_tmp_3_%(statdatenum)s as
          select /*+ MAPJOIN(tt) */ distinct tt.fgamefsk
            ...
            left join work.bud_partner_detail_tmp_2_%(statdatenum)s t5   --昨日新增活跃
        ...
        base_hql = """
          select fgamefsk
            ...
            from work.bud_partner_detail_tmp_3_%(statdatenum)s t
           where t.fpartner <> '0'
             and t.fgame_id <> -13658
           group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fversionfsk
                    ,fsource_path, fpromoter, fpartner
         """

        # 组合
        hql = (
            """ drop table if exists work.bud_partner_detail_tmp_%(statdatenum)s;
          create table work.bud_partner_detail_tmp_%(statdatenum)s as""" +
        ...
        # 组合
        hql = (
            """insert into table work.bud_partner_detail_tmp_%(statdatenum)s """ +
        ...
        # 插入目标表
        hql = """  insert overwrite table bud_dm.bud_partner_detail  partition(dt='%(statdate)s')
                   select "%(statdate)s" fdate
                     ...
                     from work.bud_partner_detail_tmp_%(statdatenum)s
                    where freg_unum + fact_unum + fpay_unum > 0
                    group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fversionfsk
                             ,fsource_path,fpromoter,fpartner;
        """
        res = self.sql_exe(hql)
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.bud_partner_detail_tmp_%(statdatenum)s;
                 drop table if exists work.bud_partner_detail_tmp_2_%(statdatenum)s;
                 drop table if exists work.bud_partner_detail_tmp_3_%(statdatenum)s;
        """
        res = self.sql_exe(hql)
...
# 生成统计实例
a = agg_bud_partner_detail(sys.argv[1:])
a()
