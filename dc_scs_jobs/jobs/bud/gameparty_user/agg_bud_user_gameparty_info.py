...
class agg_bud_user_gameparty_info(BaseStatModel):
    def create_tab(self):
        hql = """
        create table if not exists bud_dm.bud_user_gameparty_info (
               fdate               date,
               fgamefsk            bigint,
               fplatformfsk        bigint,
               fhallfsk            bigint,
               ...
               fmax_paly_time      bigint         comment '最长单局时长',
               fwin_amt            bigint         comment '赢得的游戏币',
               flose_amt           bigint         comment '输得的游戏币'
               )comment '玩牌用户信息表'
               partitioned by(dt date)
        location '/dw/bud_dm/bud_user_gameparty_info';
        """
    ...
    def stat(self):
        ...
        # 取基础数据
        hql = """--取玩牌相关指标
            drop table if exists work.bud_user_gameparty_info_tmp_1_%(statdatenum)s;
          create table work.bud_user_gameparty_info_tmp_1_%(statdatenum)s as
        ...
        base_hql = """
          select  fgamefsk
            ...
            from work.bud_user_gameparty_info_tmp_1_%(statdatenum)s t
           group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fversionfsk
         """

        # 组合
        hql = (
            """drop table if exists work.bud_user_gameparty_info_tmp_%(statdatenum)s;
          create table work.bud_user_gameparty_info_tmp_%(statdatenum)s as """ +
        ...
        # 取基础数据
        hql = """--取玩牌相关指标
            drop table if exists work.bud_user_gameparty_info_tmp_2_%(statdatenum)s;
          create table work.bud_user_gameparty_info_tmp_2_%(statdatenum)s as
          select /*+ MAPJOIN(tt) */ tt.fgamefsk
                 ,tt.fplatformfsk
                 ,tt.fhallfsk
                 ,t1.fgame_id
        ...
        base_hql = """
          select  fgamefsk
            ...
            from work.bud_user_gameparty_info_tmp_2_%(statdatenum)s t
           group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fversionfsk
         """

        # 组合
        hql = (
            """insert into table work.bud_user_gameparty_info_tmp_%(statdatenum)s """ +
            base_hql + """%(extend_group_1)s """ +
            base_hql + """%(extend_group_2)s """) % query
        ...
        # 插入目标表
        hql = """  insert overwrite table bud_dm.bud_user_gameparty_info  partition(dt='%(statdate)s')
                   select "%(statdate)s" fdate
                     ...
                     from work.bud_user_gameparty_info_tmp_%(statdatenum)s
                    group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fversionfsk;
        """
        res = self.sql_exe(hql)
        if res != 0:
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.bud_user_gameparty_info_tmp_1_%(statdatenum)s;
                 drop table if exists work.bud_user_gameparty_info_tmp_2_%(statdatenum)s;
                 drop table if exists work.bud_user_gameparty_info_tmp_%(statdatenum)s;
        """
        res = self.sql_exe(hql)
...
# 生成统计实例
a = agg_bud_user_gameparty_info(sys.argv[1:])
a()
