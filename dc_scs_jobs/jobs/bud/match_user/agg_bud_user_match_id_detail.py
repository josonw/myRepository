...
class agg_bud_user_match_id_detail(BaseStatModel):
    def create_tab(self):
        hql = """
        create table if not exists bud_dm.bud_user_match_id_detail (
               fdate                 date,
               fgamefsk              bigint,
               fplatformfsk          bigint,
               fhallfsk              bigint,
               ...
               fs_time               varchar(100)     comment '开始时间',
               fe_time               varchar(100)     comment '结束时间',
               fuse_time             bigint           comment '比赛时长秒',
        ...
        location '/dw/bud_dm/bud_user_match_id_detail';
        """
    ...
    def stat(self):
        ...
        extend_group = {'fields': ['fparty_type', 'fpname', 'fsubname', 'fgsubname', 'fmatch_id'],
                        'groups': [[1, 1, 1, 1, 1]]}
        ...
        # 取报名相关数据
        hql = """--
            drop table if exists work.bud_user_match_id_detail_tmp_1_%(statdatenum)s;
          create table work.bud_user_match_id_detail_tmp_1_%(statdatenum)s as
          select  tt.fgamefsk
                 ,tt.fplatformfsk
                 ,tt.fhallfsk
                 ,t1.fgame_id
                 ...
                 ,fitem_id          --报名物品id
                 ,fentry_fee        --报名费
                 ,case when fparty_type = '快速赛' then cast (t1.fmatch_cfg_id as string) else fmatch_id end fmatch_id        --比赛id
                 ,ffirst            --首次报名某一级比赛场
        ...
        hql = """
          select "%(statdate)s" fdate
            ...
            from work.bud_user_match_id_detail_tmp_1_%(statdatenum)s t
           where hallmode = %(hallmode)s
           group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fversionfsk
                    ,fparty_type,fpname,fsubname,fgsubname,fmatch_id
         """
        self.sql_template_build(sql=hql, extend_group=extend_group)

        # 组合
        hql = """
            drop table if exists work.bud_user_match_id_detail_tmp_%(statdatenum)s;
          create table work.bud_user_match_id_detail_tmp_%(statdatenum)s as
          %(sql_template)s;
         """
        ...
        # 取参赛相关数据
        hql = """--
            drop table if exists work.bud_user_match_id_detail_tmp_2_%(statdatenum)s;
          create table work.bud_user_match_id_detail_tmp_2_%(statdatenum)s as
          select  tt.fgamefsk
                 ,tt.fplatformfsk
                 ,tt.fhallfsk
                 ,t1.fgame_id
                 ...
                 ,ftbl_id
                 ,finning_id
                 ,concat_ws('0', t1.fmatch_id, cast (t1.fuid as string)) fmatch_uid  --match_id和uid组合字段
                 ,fpname            --牌局一级分类
                 ,fsubname          --牌局二级分类
                 ,fgsubname         --牌局三级分类
                 ,fparty_type       --牌局类型
        ...
        hql = """
          select "%(statdate)s" fdate
                 ...
                 ,count(distinct fmatch_uid) fmatch_cnt    --参赛人次
                 ...
                 ,count(distinct case when is_pay = 1 then fmatch_uid end) fmatch_pay_cnt        --当日付费用户参赛次数
                 ,0 fmatch_pay_income     --当日付费用户参赛用户付费金额
                 ,0 fquit_unum             --取消报名人数
                 ,0 fquit_cnt             --取消报名人次
                 ,0 fz_quit_unum          --主动取消报名人数
                 ...
                 ,0 fbegin_cnt            --开赛次数
                 ,0 fbegin_suss_cnt       --开赛成功次数
            from work.bud_user_match_id_detail_tmp_2_%(statdatenum)s t
           where hallmode = %(hallmode)s
           group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fversionfsk
                    ,fparty_type,fpname,fsubname,fgsubname,fmatch_id
         """
        ...
        # 组合
        hql = """
            insert into table work.bud_user_match_id_detail_tmp_%(statdatenum)s
          %(sql_template)s;
         """
        ...
        # 取发放消耗相关数据
        hql = """--
            drop table if exists work.bud_user_match_id_detail_tmp_3_%(statdatenum)s;
          create table work.bud_user_match_id_detail_tmp_3_%(statdatenum)s as
          select  tt.fgamefsk
                 ,tt.fplatformfsk
                 ,tt.fhallfsk
        ...
        hql = """
          select "%(statdate)s" fdate
                 ...
                 ,0 fbegin_cnt            --开赛次数
                 ,0 fbegin_suss_cnt       --开赛成功次数
            from work.bud_user_match_id_detail_tmp_3_%(statdatenum)s t
           where hallmode = %(hallmode)s
           group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fversionfsk
                    ,fparty_type,fpname,fsubname,fgsubname,fmatch_id
         """
        ...
        # 汇总
        hql = """
            insert overwrite table bud_dm.bud_user_match_id_detail partition(dt='%(statdate)s')
        select '%(statdate)s' fdate
               ,fgamefsk
               ,fplatformfsk
               ,fhallfsk
          ...
          from (select  fgamefsk
                  ...
                  from (select  fgamefsk
                               ...
                               ,fbegin_cnt
                               ,fbegin_suss_cnt
                          from work.bud_user_match_id_detail_tmp_%(statdatenum)s
                         union all
                        select  fgamefsk
                               ,fplatformfsk
                               ,fhallfsk
                 ...
                 group by fgamefsk
                         ...
                         ,t.fgsubname
                         ,fmatch_id) t
                  left join (select fmatch_id
                                    ,min(fbigin_at) fs_time
                                    ,max(fend_at) fe_time
                                    ,sum(unix_timestamp(fend_at)-unix_timestamp(fbigin_at)) fuse_time
                                    ,count(distinct fmatch_id) fbegin_cnt
                                    ,count(distinct case when fis_fail = 0 then fmatch_id end) fbegin_suss_cnt
                               from dim.match_config t
                              where dt = '%(statdate)s'
                              group by fmatch_id
                            ) t2
                    on t.fmatch_id = t2.fmatch_id
         """
        ...
        # 插入结果表
        hql = """--
        insert overwrite table bud_dm.bud_user_match_gsubgame_detail_match_time
            ...
            select '%(statdate)s' fdate
                   ...
                   ,fsubname
                   ,fgsubname
                   ,sum(fuse_time) fmatch_time
              from bud_dm.bud_user_match_id_detail t
             where t.dt="%(statdate)s"
             group by fgamefsk,fplatformfsk,fhallfsk,fsubgamefsk,fterminaltypefsk,fversionfsk,fparty_type,fpname,fsubname,fgsubname
        """
        res = self.sql_exe(hql)
        if res != 0:
            return res
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.bud_user_match_id_detail_tmp_1_%(statdatenum)s;
                 drop table if exists work.bud_user_match_id_detail_tmp_2_%(statdatenum)s;
                 drop table if exists work.bud_user_match_id_detail_tmp_3_%(statdatenum)s;
                 drop table if exists work.bud_user_match_id_detail_tmp_%(statdatenum)s;
        """
...
# 生成统计实例
a = agg_bud_user_match_id_detail(sys.argv[1:])
a()
