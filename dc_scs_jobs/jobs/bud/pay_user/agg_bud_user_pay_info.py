...
class agg_bud_user_pay_info(BaseStatModel):
    def create_tab(self):
        hql = """
        create table if not exists bud_dm.bud_user_pay_info (
               fdate               date,
               fgamefsk            bigint,
               fplatformfsk        bigint,
               fhallfsk            bigint,
               ...
               frupt_pay_income    decimal(20,2)  comment '破产(状态)付费额度',
               forder_unum         bigint         comment '下单用户数'
               )comment '付费用户信息表'
               partitioned by(dt date)
        location '/dw/bud_dm/bud_user_pay_info';
        """
    ...
    def stat(self):
        ...
        # 取基础数据
        hql = """--取订单相关指标
            drop table if exists work.bud_user_pay_info_tmp_%(statdatenum)s;
          create table work.bud_user_pay_info_tmp_%(statdatenum)s as
        ...
        base_hql = """
          select "%(statdate)s" fdate
            ...
            from work.bud_user_pay_info_tmp_%(statdatenum)s t
           group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fversionfsk
         """

        # 组合
        hql = (
            """insert overwrite table bud_dm.bud_user_pay_info
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.bud_user_pay_info_tmp_%(statdatenum)s;
        """
        res = self.sql_exe(hql)
        if res != 0:
            return res
...
# 生成统计实例
a = agg_bud_user_pay_info(sys.argv[1:])
a()
