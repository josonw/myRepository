...
class agg_bud_user_payscene_product_detail(BaseStatModel):
    def create_tab(self):
        hql = """
        create table if not exists bud_dm.bud_user_payscene_product_detail (
               fdate               date,
               fgamefsk            bigint,
               fplatformfsk        bigint,
               fhallfsk            bigint,
               ...
               fproduct_name       varchar(100)   comment '商品名称',
        ...
        location '/dw/bud_dm/bud_user_payscene_product_detail';
        """
    ...
    def stat(self):
        ...
        extend_group = {'fields': ['fpay_scene', 'fproduct_id', 'fproduct_name'],
                        'groups': [[1, 1, 1],
                                   [0, 1, 1]]}
        ...
        # 取基础数据
        hql = """--取订单相关指标
            drop table if exists work.bud_user_payscene_product_detail_tmp_%(statdatenum)s;
          create table work.bud_user_payscene_product_detail_tmp_%(statdatenum)s as
          select /*+ MAPJOIN(tt) */ distinct tt.fgamefsk
                 ...
                 ,coalesce(t1.fproduct_id,'%(null_str_report)s')  fproduct_id
                 ,coalesce(t1.fproduct_name,'%(null_str_report)s')  fproduct_name
        ...
        hql = """
          select "%(statdate)s" fdate
                 ...
                 ,coalesce(fproduct_id,'%(null_str_group_rule)s') fproduct_id
                 ,coalesce(fproduct_name,'%(null_str_group_rule)s') fproduct_name
            ...
            from work.bud_user_payscene_product_detail_tmp_%(statdatenum)s t
        ...
        hql = """
         insert overwrite table bud_dm.bud_user_payscene_product_detail
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.bud_user_payscene_product_detail_tmp_%(statdatenum)s;
        """
        res = self.sql_exe(hql)
        if res != 0:
            return res
...
# 生成统计实例
a = agg_bud_user_payscene_product_detail(sys.argv[1:])
a()
