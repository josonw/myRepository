...
class agg_bud_user_entrance_dis(BaseStatModel):
    def create_tab(self):
        hql = """
        create table if not exists bud_dm.bud_user_entrance_dis (
               fdate                 date,
               fgamefsk              bigint,
               fplatformfsk          bigint,
               fhallfsk              bigint,
        ...
        location '/dw/bud_dm/bud_user_entrance_dis';
        """
    ...
    def stat(self):
        ...
        # 取基础数据
        hql = """--
            drop table if exists work.bud_user_entrance_dis_tmp_1_%(statdatenum)s;
          create table work.bud_user_entrance_dis_tmp_1_%(statdatenum)s as
          select t1.fbpid
                 ,fuid
        ...
        # 取基础数据
        hql = """--
            drop table if exists work.bud_user_entrance_dis_tmp_2_%(statdatenum)s;
          create table work.bud_user_entrance_dis_tmp_2_%(statdatenum)s as
          select  tt.fgamefsk
                 ,tt.fplatformfsk
                 ,tt.fhallfsk
            ...
            left join work.bud_user_entrance_dis_tmp_1_%(statdatenum)s t2
        ...
        base_hql = """
          select "%(statdate)s" fdate
            ...
            from work.bud_user_entrance_dis_tmp_2_%(statdatenum)s t
        ...
        # 组合
        hql = (
            """insert overwrite table bud_dm.bud_user_entrance_dis
        ...
        # 取基础数据
        hql = """--
            drop table if exists work.bud_user_entrance_dis_tmp_3_%(statdatenum)s;
          create table work.bud_user_entrance_dis_tmp_3_%(statdatenum)s as
          select  tt.fgamefsk
                 ,tt.fplatformfsk
                 ,tt.fhallfsk
        ...
        base_hql = """
          select "%(statdate)s" fdate
                 ...
                 ,count(distinct case when type = 1 and type_num > 0 then fuid end) reg_pay_unum           --注册付费用户数
                 ,sum(case when type = 5 then type_num end) pay_income             --付费总额
            from work.bud_user_entrance_dis_tmp_3_%(statdatenum)s t
           group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fversionfsk
                    ,fentrance_id
         """
        ...
        # 组合
        hql = (
            """insert into table bud_dm.bud_user_entrance_dis
                      partition(dt='%(statdate)s') """ +
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.bud_user_entrance_dis_tmp_1_%(statdatenum)s;
                 drop table if exists work.bud_user_entrance_dis_tmp_2_%(statdatenum)s;
                 drop table if exists work.bud_user_entrance_dis_tmp_3_%(statdatenum)s;
        """
        res = self.sql_exe(hql)
...
# 生成统计实例
a = agg_bud_user_entrance_dis(sys.argv[1:])
a()
