...
class agg_bud_user_version_terminal_dis(BaseStatModel):
    def create_tab(self):
        hql = """
        create table if not exists bud_dm.bud_user_version_terminal_dis (
               fdate                 date,
               fgamefsk              bigint,
               fplatformfsk          bigint,
               fhallfsk              bigint,
        ...
        location '/dw/bud_dm/bud_user_version_terminal_dis';
        """
    ...
    def stat(self):
        ...
        # 取基础数据--设备号
        hql = """--
            drop table if exists work.bud_user_version_terminal_dis_tmp_0_%(statdatenum)s;
          create table work.bud_user_version_terminal_dis_tmp_0_%(statdatenum)s as
          select t1.fbpid
                 ,fuid
                 ,max(fm_imei) fm_imei
        ...
        # 取基础数据--版本
        hql = """--
            drop table if exists work.bud_user_version_terminal_dis_tmp_1_%(statdatenum)s;
          create table work.bud_user_version_terminal_dis_tmp_1_%(statdatenum)s as
          select t1.fbpid
                 ,fuid
                 ,max(fversion_info) fversion_info
            from (select fbpid, fuid, coalesce(fversion_info, '0') fversion_info
                    from dim.user_login_additional
                   where dt= '%(statdate)s'
                   union all
                  select fbpid, fuid, coalesce(fversion_info, '0') fversion_info
                    from dim.reg_user_main_additional
                   where dt= '%(statdate)s'
                 ) t1
            join dim.bpid_map_bud tt
        ...
        # 取基础数据
        hql = """--
            drop table if exists work.bud_user_version_terminal_dis_tmp_2_%(statdatenum)s;
          create table work.bud_user_version_terminal_dis_tmp_2_%(statdatenum)s as
          select  tt.fgamefsk
                 ,tt.fplatformfsk
                 ,tt.fhallfsk
            ...
            left join work.bud_user_version_terminal_dis_tmp_1_%(statdatenum)s t2
              on t1.fbpid = t2.fbpid
             and t1.fuid = t2.fuid
            left join work.bud_user_version_terminal_dis_tmp_0_%(statdatenum)s t4
          ...
          select /*+ MAPJOIN(tt) */ distinct tt.fgamefsk
            ...
            left join work.bud_user_version_terminal_dis_tmp_1_%(statdatenum)s t2
              on t1.fbpid = t2.fbpid
             and t1.fuid = t2.fuid
            left join work.bud_user_version_terminal_dis_tmp_0_%(statdatenum)s t3
              on t1.fbpid = t3.fbpid
             and t1.fuid = t3.fuid
            join dim.bpid_map_bud tt
              on t1.fbpid = tt.fbpid
        ...
        base_hql = """
          select "%(statdate)s" fdate
            ...
            from work.bud_user_version_terminal_dis_tmp_2_%(statdatenum)s t
        ...
        # 组合
        hql = (
            """insert overwrite table bud_dm.bud_user_version_terminal_dis
        ...
        # 取基础数据
        hql = """--
            drop table if exists work.bud_user_version_terminal_dis_tmp_3_%(statdatenum)s;
          create table work.bud_user_version_terminal_dis_tmp_3_%(statdatenum)s as
          select  tt.fgamefsk
                 ,tt.fplatformfsk
                 ,tt.fhallfsk
        ...
        base_hql = """
          select "%(statdate)s" fdate
                 ...
                 ,count(distinct case when type = 6 then fm_imei end) first_pay_unum         --首付用户数
                 ,count(distinct case when type = 1 and type_num > 0 then fm_imei end) reg_pay_unum           --注册付费用户数
            from work.bud_user_version_terminal_dis_tmp_3_%(statdatenum)s t
           group by fgamefsk,fplatformfsk,fhallfsk,fgame_id,fterminaltypefsk,fversionfsk
                    ,fversion_info
         """
        ...
        # 组合
        hql = (
            """insert into table bud_dm.bud_user_version_terminal_dis
                      partition(dt='%(statdate)s') """ +
        ...
        # 统计完清理掉临时表
        hql = """drop table if exists work.bud_user_version_terminal_dis_tmp_0_%(statdatenum)s;
                 drop table if exists work.bud_user_version_terminal_dis_tmp_1_%(statdatenum)s;
                 drop table if exists work.bud_user_version_terminal_dis_tmp_2_%(statdatenum)s;
                 drop table if exists work.bud_user_version_terminal_dis_tmp_3_%(statdatenum)s;
        """
...
# 生成统计实例
a = agg_bud_user_version_terminal_dis(sys.argv[1:])
a()
