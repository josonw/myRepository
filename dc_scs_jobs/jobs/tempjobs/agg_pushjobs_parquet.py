...
class agg_pushjobs_parquet(BaseStat):
    ...
    def version(self, vfield, selectedValue, inputValue):
        ...
        if selectedValue in (">=","<="):
            for i,v in enumerate(inputValue.split(".")):
                vstr="coalesce(cast (regexp_extract(%s,'^([0-9])+\\.([0-9])+\\.([0-9]+)',%s) as int),0)%s%s"%(vfield,i+1,selectedValue,v)
                verlist.append(vstr)
            ...
            for j in range(1,len(inv)+1):
                ...
                for i in range(0,j-1):
                    vstr="coalesce(cast (regexp_extract(%s,'^([0-9])+\\.([0-9])+\\.([0-9]+)',%s) as int),0)=%s"%(vfield,i+1,inv[i])
                    temp.append(vstr)
                selectedv="coalesce(cast (regexp_extract(%s,'^([0-9])+\\.([0-9])+\\.([0-9]+)',%s) as int),0)%s%s"%(vfield,j,selectedValue,inv[j-1])
                if temp:
    ...
    def assembly_where(self, query, isreverse=False):
        ...
        if isreverse:
            bpidstr = " fbpid='%s' and fpush_platform<>'%s' " %(query.get('bpid'), query.get("fpush_platform"))
        ...
        if str(query.get("user_type"))=='1':
            ...
            conditions = "%s  and fuid in (%s)  "%(bpidstr,uidstr)
            ...
            if data.get('18'):
                sinup_info= data.get('18')
                # 年龄--无数据
                formatstr = " %s>=%s and %s<%s "%("fage", sinup_info.get('begin'), "fage", sinup_info.get('end'))
                condition.append(formatstr)

            if data.get('19'):
                sinup_info= data.get('19')
                # 生日--无数据
                formatstr = " %s>=%s and %s<%s "%("fbrith", sinup_info.get('begin'), "fbrith", sinup_info.get('end'))
                condition.append(formatstr)

            if data.get('20'):
                sinup_info= data.get('20')
                # 累计玩牌天数
                formatstr = " %s>=%s and %s<%s "%("fparty_num_day", sinup_info.get('begin'), "fparty_num_day", sinup_info.get('end'))
                condition.append(formatstr)

            if data.get('21'):
                sinup_info= data.get('21')
                # 上一次约牌房玩牌局数
                formatstr = " %s>=%s and %s<%s "%("fy_party_num", sinup_info.get('begin'), "fy_party_num", sinup_info.get('end'))
                condition.append(formatstr)

            if data.get('22'):
                sinup_info= data.get('22')
                # 累计约牌房玩牌局数
                formatstr = " %s>=%s and %s<%s "%("fy_party_num_total", sinup_info.get('begin'), "fy_party_num_total", sinup_info.get('end'))
                condition.append(formatstr)

            if data.get('23'):
                sinup_info= data.get('23')
                # 累计约牌房玩牌天数
                formatstr = " %s>=%s and %s<%s "%("fy_party_num_day", sinup_info.get('begin'), "fy_party_num_day", sinup_info.get('end'))
                condition.append(formatstr)

            if data.get('24'):
                sinup_info= data.get('24')
                # 上一次付费额度
                formatstr = " %s>=%s and %s<%s "%("fpay_num", sinup_info.get('begin'), "fpay_num", sinup_info.get('end'))
                condition.append(formatstr)

            if data.get('25'):
                sinup_info= data.get('25')
                # 上一次破产次数
                formatstr = " %s>=%s and %s<%s "%("frupt_num", sinup_info.get('begin'), "frupt_num", sinup_info.get('end'))
                condition.append(formatstr)

            if data.get('26'):
                sinup_info= data.get('26')
                # 累计破产次数
                formatstr = " %s>=%s and %s<%s "%("frupt_num_total", sinup_info.get('begin'), "frupt_num_total", sinup_info.get('end'))
                condition.append(formatstr)
    ...
    def assembly_sql(self, query):
        if '_except_' in query.get("fpush_platform"):
            ...
            tempsql = """insert overwrite table pushdb.push_user_parquet
                              partition(fpushid = '%(jobname)s')
                      select a.fuid, a.ftoken, a.fpush_platform from
                             (
                              select fuid, ftoken, fpush_platform
                                from dim.parquet_user_push %(conditions1)s
                               group by fuid, ftoken, fpush_platform
                             )  a
                             ...
                             (
                              select fuid, ftoken, fpush_platform
                                from dim.parquet_user_push %(conditions2)s
                               group by fuid, ftoken, fpush_platform
                              ) b
                           on a.ftoken=b.ftoken
                        where coalesce(b.ftoken,'')='' ;
                       select ftoken from
                       pushdb.push_user_parquet
    ...
    def stat(self):
        ...
        if data.get('jobtype','') in ('pushjobs'):
            ...
            if str(query.get("user_type"))=='4':
                res = impala.exe_sql("INVALIDATE METADATA")  #Hive 中 create, drop, 或 alter 了一个表或其他对象,impala需要刷新
                res = impala.exe_sql("refresh {tb_name}".format(tb_name=query.get("tb_name")))   #Hive 中 load, insert, 或 change 了现存表中的数据 (乃至直接通过 HDFS 操作如 hdfs 命令)，impala需要刷新
            res = impala.exe_sql(sql_temp)   # 执行统计sql，将数据插入到pushdb.push_user，这里不适用命令行，因为命令行长度有限制，容易出错
            ...
            # 获取推送的数量
            hql = """select count(distinct ftoken) push_num
                       from pushdb.push_user_parquet
                      where fpushid = '%(jobname)s'""" % data
            print hql

            impala = ImpalaSql(host='10.30.101.104')
            push_num = impala.query(hql)
            print push_num
            url = "https://pushd.boyaa.com/pushd/count?push_id=%(jobname)s&push_num=%(push_num)s&success_num=0&click_num=0&appear_num=0"%{"jobname":data['jobname'], "push_num": list(push_num)[0]['push_num']}
...
#生成统计实例
a = agg_pushjobs_parquet(statDate)
a()
