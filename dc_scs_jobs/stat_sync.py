...
class ShellExec(object):
    ...
    #host = config.HIVE_DB_IP #由于hive客户端由94更改为216故修改
    #port = config.HIVE_NODE_SSH_PORT
    #user = config.HIVE_NODE_SSH_USER
    #pkey = config.HIVE_NODE_SSH_PKEY

    def __init__(self,host = config.HIVE_NODE_SSH_IP,port = config.HIVE_NODE_SSH_PORT,\
                user = config.HIVE_NODE_SSH_USER,pkey = config.HIVE_NODE_SSH_PKEY):
        self.host = host
        self.port = port
        self.user = user
        self.pkey = pkey
...
class Sync(object):
    ...
    def _get_table_info(self):
        ...
        self.fieldstr = self.get_table_fields(self.schema,self.tbl_name)
    ...
    def get_sqoop_import_all_cmd(self, table_name):
        ...
        cmds = []
        tbl_dir = '/dw/analysis/%s' % table_name
        cmd = sqoop_cmd % self._get_sqoop_dict('', table_name, tbl_dir, self.schema)
        cmds.append(cmd)
        return cmds

    def hive_concat(self, table_name, dates, _type):
        shell = ShellExec(host = config.HIVE_DB_IP)
        if _type == "line":
