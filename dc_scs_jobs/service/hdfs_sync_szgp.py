...
pg_db = pgdb.Connection(host      = config.PGCLUSTER_SZHOST,
...
class Sync(object):
    ...
    def __init__(self, sync_type='export', table=None):
        ...
        create external table  _%(pg_schema)s_%(pg_tbl_name)s
        (%(hfields)s)
        location(%(files_dir)s)
        format 'text' (delimiter E'\x01' escape 'OFF');
        insert into %(pg_schema)s.%(pg_tbl_name)s select %(pfields)s from _%(pg_schema)s_%(pg_tbl_name)s
        """
    ...
    def _get_table_info(self,pg_table):
        ...
        self.namenodehost = "10.30.101.92:8020"
    ...
    def _get_sql_dict(self, hfields, pfields, table, hive_dir, pg_schema):
    ...
    def get_create_extable_export_sql(self, table_name, dates):
        ...
        cmd = create_tblsql % self._get_sql_dict(hfields,pfields,self.pg_tbl_name,tbl_dir,self.pg_schema)
    ...
    def get_create_sql_cmd(self, table_name):
        ...
        cmd = create_tblsql % self._get_sql_dict(hfields,'*', self.pg_tbl_name, tbl_dir, self.pg_schema)
        return cmd
    ...
    def get_create_extable_import_sql(self, table_name):
        ...
        cmd = create_tblsql % self._get_sql_dict(hfields,'*', table_name, tbl_dir,self.pg_schema)
        return cmd
